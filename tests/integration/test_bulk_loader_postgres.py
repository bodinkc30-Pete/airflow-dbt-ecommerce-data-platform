...
def test_product_card_verified_load_contract_maps_all_business_columns(
    postgres_connection,
    tmp_path: Path,
) -> None:
    source_file = tmp_path / "product_card_verified_contract_integration.xlsx"
    source_file.write_bytes(b"synthetic-product-card-contract")
    ...
    row[source_columns[0]] = "2026-09-08"
    dataframe = pd.DataFrame([row], columns=source_columns)
    lineage = LineageMetadata(
        source_file=source_file.name,
        batch_id="batch_product_card_contract_integration",
        file_hash=metadata.file_hash_sha256,
        pipeline_run_id=ingestion_run_id,
        ingestion_file_id=registered_file.ingestion_file_id,
        ingested_at=datetime(2026, 9, 8, tzinfo=UTC),
    ...
    assert result.source_name == "product_card_traffic"
    assert result.target_table == "raw.product_card_daily"
    assert result.rows_attempted == 1
    assert result.rows_loaded == 1
